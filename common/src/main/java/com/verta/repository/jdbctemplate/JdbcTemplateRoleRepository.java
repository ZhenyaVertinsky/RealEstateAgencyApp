...
public class JdbcTemplateRoleRepository implements RoleRepositoryInterface{
    ...
    public Role findById(Long id) {
        return jdbcTemplate.queryForObject("select * from entity.roles where id = " + id, rowMapper);
    }
    ...
    public List<Role> findAll(int limit, int offset) {
        return jdbcTemplate.query("select * from entity.roles limit " + limit + " offset " + offset, rowMapper);
    }

    @Override
    public Role create(Role object) {
        final String insertQuery =
                "insert into entity.roles (role_name, creation_date, modification_date) " +
        ...
        Long lastInsertId = namedParameterJdbcTemplate.query("SELECT currval('entity.roles_id_seq') as last_id",
                resultSet -> {
                    resultSet.next();
    ...
    public Long delete(Long id) {
        jdbcTemplate.update("delete from entity.roles where id = " + id);
        return id;
    }
